...
class DepthFusionModule5(nn.Module):
    def __init__(self, embed_dims,num_heads,weight):
        super(DepthFusionModule5, self).__init__()
        self.embed_dims = embed_dims
        self.gamma = Scale(0)
        self.se = DepthFusionModule4(embed_dims)
        self.dsaadd = DepthFusionModule2(embed_dims,num_heads,norm_cfg=dict(type='BN'),weight=weight)
        # self.norm = build_norm_layer(norm_cfg, embed_dims)[1]
        # self.spatial_gap = nn.Conv2d(embed_dims, 1, kernel_size=1, bias=True)
        self.weight = weight
        
    def forward(self, color, depth):
        x = self.se.forward(color, depth)
        # x = torch.cat([color, depth], dim=1)
        # out = super(DepthFusionModule5, self).forward(x)[:, :self.embed_dims]
        out = self.dsaadd.forward(x,depth)
        return out
    
class DepthFusionModule6(nn.Module):
    def __init__(self, embed_dims, num_heads):
        super(DepthFusionModule6, self).__init__()
        self.embed_dims = embed_dims
        self.gamma = Scale(0)
        self.se = DepthFusionModule4(embed_dims)
        # self.dsaadd = DepthFusionModule2(embed_dims,num_heads,norm_cfg=dict(type='BN'),weight=weight)
        ...
        # self.spatial_gap = nn.Conv2d(embed_dims, 1, kernel_size=1, bias=True)
        # self.weight = weight
        self.dsaconcat = DepthFusionModule1(embed_dims, num_heads)
        
    def forward(self, color, depth):
        x = self.se.forward(color, depth)
        # x = torch.cat([color, depth], dim=1)
        # out = super(DepthFusionModule5, self).forward(x)[:, :self.embed_dims]
        out = self.dsaconcat.forward(x,depth)
        return out
...
class EDFT(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(self.num_stages):
            ...
            elif attention_type == 'ca-dsa-add':
                self.dfms.append(DepthFusionModule5(embed_dims_i, self.num_heads[i], weight=self.weight))
            elif attention_type == 'ca-dsa-concat':
                self.dfms.append(DepthFusionModule6(embed_dims_i, self.num_heads[i]))
            else:
    ...
    def forward(self, x):
        ...
        #     print(i.shape)
        d_outs=[]
        d_outs.append(torch.reshape(x[:,3:5],[1, 32, 128, 128]))
        d_outs.append(torch.reshape(x[:,5],[1, 64, 64, 64]))
        stage3 = torch.reshape(x[:,6],[2, 128, 32, 32])[0].unsqueeze(0)
        # pad [1,128,32,32]->[1,160,32,32]
        stage3 = F.pad(stage3,(0,0,0,0,16,16),mode="constant",value=0)
        # print(stage3.shape)
        d_outs.append(stage3)
        d_outs.append(torch.reshape(x[:,7],[4, 256, 16, 16])[0].unsqueeze(0))
        # d_outs = self.depth(d)
